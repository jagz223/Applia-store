/**
 * Texto legal: condiciones de uso (prestadores y clientes) — Applia.
 * Se muestra en /politics y en el modal obligatorio hasta aceptar (prestadores).
 */
export function ProviderTermsOfUseContent() {
  return (
    <div className="space-y-4 text-sm leading-relaxed text-foreground">
      <p className="font-semibold text-center text-base">
        ESTATUTO DE CONDICIONES DE USO, RÉGIMEN ECONÓMICO Y PROTOCOLOS OPERATIVOS – APPLIA
      </p>

      <section className="space-y-2">
        <p className="font-semibold">PARTE I: CONTRATO DE ADHESIÓN PARA PRESTADORES Y COMERCIOS</p>
        <p>
          <strong>1. IDENTIFICACIÓN DE LAS PARTES</strong>
          <br />
          El presente documento regula la relación jurídica entre el operador de la plataforma digital Applia (en
          adelante, &quot;La Plataforma&quot;), con domicilio y operación en la República del Ecuador, y el usuario que
          se registre bajo el perfil de &quot;Socio-Colaborador&quot; o &quot;Comercio&quot; (en adelante, &quot;El
          Prestador&quot;).
        </p>
        <p>
          <strong>2. OBJETO Y CATEGORÍAS DE SERVICIOS</strong>
          <br />
          El objeto es el uso de la infraestructura de Applia como <strong>canal de difusión y contacto</strong>: publicar
          perfiles y servicios, facilitar la comunicación (incluido el chat) y herramientas operativas. La contratación y
          el cobro del servicio o producto entre el cliente y el prestador se acuerdan <strong>entre esas partes</strong>;
          Applia no actúa como intermediario financiero ni procesa esos pagos en la aplicación. Áreas cubiertas:
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <strong>Servidores Profesionales y Técnicos:</strong> Consultoría, reparaciones, mantenimiento y oficios
            especializados.
          </li>
          <li>
            <strong>Transporte y Movilidad:</strong> Choferes de taxis, tricimotos y servicios de transporte privado.
          </li>
          <li>
            <strong>Logística y Delivery:</strong> Mensajería urbana y reparto de última milla.
          </li>
          <li>
            <strong>Marketplace:</strong> Establecimientos comerciales para la exhibición y venta de bienes físicos.
          </li>
        </ul>
        <p>
          <strong>3. NATURALEZA DEL VÍNCULO JURÍDICO</strong>
          <br />
          La relación entre las partes es estrictamente mercantil y civil. El Prestador declara que actúa como un
          agente económico independiente, con sus propios medios, herramientas y vehículos. Bajo ninguna circunstancia
          existe relación laboral, de dependencia o subordinación con La Plataforma ni con Applia en calidad de
          empleador.
        </p>
        <p>
          <strong>4. RÉGIMEN ECONÓMICO Y TARIFARIO</strong>
          <br />
          El Prestador acepta las siguientes condiciones comerciales por el uso del ecosistema:
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <strong>Cuota por visibilidad y uso de la plataforma:</strong> Applia puede cobrar una{" "}
            <strong>suscripción o cuota periódica fija</strong> por mantener activa la publicación del perfil y los
            servicios, y por el acceso a las funciones de la aplicación. El monto, periodicidad y condiciones concretas se
            informan <strong>en la propia aplicación</strong> y pueden variar según categoría o tipo de prestador. Esa
            cuota <strong>no sustituye ni incluye</strong> el precio del servicio que el Prestador cobre al cliente final.
          </li>
          <li>
            <strong>Sin cobro por transacción a través de Applia:</strong> La Plataforma{" "}
            <strong>no retiene, no custodia ni liquida</strong> entre las partes el pago del servicio contratado entre
            cliente y Prestador. Los medios, forma y momento del pago son acordados directamente entre ellos (incluido el
            uso del chat u otros canales que faciliten el contacto).
          </li>
          <li>
            <strong>Precios de referencia, regateo (taxi, delivery y análogos):</strong> En categorías donde la
            aplicación muestre <strong>precios base o de referencia</strong>, dichos importes son orientativos. El
            cliente y el conductor o repartidor pueden <strong>ajustar el precio</strong> mediante las funciones de
            regateo o negociación previstas, complementadas con el chat, hasta alcanzar un acuerdo.{" "}
            <strong>Applia no cobra comisión ni tarifa sobre ese acuerdo</strong>; el precio final y su cumplimiento son
            responsabilidad exclusiva de las partes.
          </li>
        </ul>
        <p>
          <strong>4.1. Cláusula de Actualización de Valores:</strong>
          <br />
          Applia se reserva la facultad de ajustar la cuota de suscripción o visibilidad y demás tarifas propias de la
          Plataforma, con los siguientes criterios orientativos:
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>Inversión en mejoras de ciberseguridad y nuevas funciones de la aplicación Applia.</li>
          <li>
            Ajustes por inflación (IPC) o incrementos en costos de infraestructura, software y operación del servicio.
          </li>
          <li>Inversión en campañas de marketing para beneficio de la comunidad de Prestadores.</li>
        </ul>
        <p>
          <strong>5. RÉGIMEN DISCIPLINARIO Y SANCIONES</strong>
          <br />
          El incumplimiento de los estándares operativos dará lugar a las siguientes medidas:
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <strong>Infracciones Leves (Suspensión 1-3 días):</strong> Cancelaciones injustificadas, retrasos
            reiterados o calificación inferior a 3.8 estrellas.
          </li>
          <li>
            <strong>Infracciones Graves (Suspensión 15-30 días):</strong> Trato inadecuado al usuario o incumplimiento de
            protocolos técnicos de seguridad.
          </li>
          <li>
            <strong>Infracciones Muy Graves (Expulsión Definitiva):</strong>
          </li>
        </ul>
        <ul className="list-disc pl-5 ml-4 space-y-1">
          <li>
            <strong>Fraude:</strong> Suplantación de identidad o entrega de productos falsificados.
          </li>
          <li>
            <strong>Seguridad:</strong> Acoso de cualquier índole o prestación de servicios bajo efectos de sustancias.
          </li>
        </ul>
      </section>

      <section className="space-y-2">
        <p className="font-semibold">PARTE II: INFORMACIÓN PARA EL CLIENTE FINAL (CONTACTO, PRECIOS Y PAGOS)</p>
        <p>
          Applia es principalmente un <strong>espacio para conocer prestadores</strong>, ver información publicada y{" "}
          <strong>ponerse en contacto</strong> (incluido el chat). El contrato de prestación del servicio o entrega del
          bien lo celebran <strong>el cliente y el prestador entre sí</strong>; el pago acordado se realiza por medios y
          condiciones que ellos definan. <strong>Applia no mueve ni custodia el dinero de esa operación</strong> dentro
          de la aplicación en este modelo de uso.
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <strong>Precio acordado:</strong> En servicios como taxi o delivery pueden mostrarse precios base o de
            referencia; el importe final puede modificarse mediante <strong>regateo o negociación</strong> y el chat,
            hasta que cliente y conductor o repartidor estén de acuerdo.
          </li>
          <li>
            <strong>Verificación:</strong> Cuando corresponda, Applia puede exigir documentación a los Prestadores con
            fines de verificación; ello no sustituye la diligencia del cliente al contratar.
          </li>
          <li>
            <strong>Limitación:</strong> Las herramientas de la app (mensajería, ubicación u otras) son auxiliares. Salvo
            disposición legal imperativa, Applia no garantiza el resultado del servicio ni asume obligación de
            reembolso por disputas comerciales o de pago entre cliente y prestador.
          </li>
        </ul>
        <p>
          <strong>⚠️ ADVERTENCIA:</strong> Cualquier acuerdo económico o prestacional queda bajo responsabilidad de las
          partes que contratan. Se recomienda dejar constancia por los canales que utilicen y verificar identidad y
          condiciones antes de pagar o recibir el servicio.
        </p>
      </section>

      <section className="space-y-2">
        <p className="font-semibold">PARTE III: REQUISITOS DE VERIFICACIÓN PARA PRESTADORES</p>
        <p>
          Para la activación o mantenimiento de la cuenta de Prestador, Applia puede solicitar y validar en el sistema la
          siguiente documentación (según políticas vigentes):
        </p>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            <strong>Identidad:</strong> Cédula de Identidad legible (ambos lados).
          </li>
          <li>
            <strong>Tributario:</strong> RUC o RIMPE vigente.
          </li>
        </ul>
      </section>
    </div>
  );
}
