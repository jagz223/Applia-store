...
export async function assertStoreCategoryIds(
  ...
  const categories = await genFebStorage.listStoreCategories(storeId);
...
export async function syncCategoryProductMembership(
  ...
  for (const product of products) {
    ...
    await genFebStorage.updateStoreProduct(storeId, product.id, { categoryIds: nextIds });
  }
}
...
export async function removeCategoryFromAllProducts(storeId: number, categoryId: number): Promise<void> {
  ...
  for (const product of products) {
    ...
    await genFebStorage.updateStoreProduct(storeId, product.id, {
